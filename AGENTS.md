# pi-key-pool — Agent 规范

## 项目一句话定位
Pi 扩展：API Key 池管理器——会话级绑定轮转、失败冷却恢复、智能重试、错误分级。npm 包 `pi-key-pool`。

## 先读文档顺序
1. 本文件
2. `README.md`（为什么、功能表、Quick Start）

## 技术栈
TypeScript Pi extension。npm 发包。

## 代码边界与安全边界
- Key 配置走 `keys.json`，不硬编码
- 不把 key 写进日志（debug 模式也不暴露完整 key）
- 错误分级：capacity(30s) / quota(5min) / network(不切换)

## 验证方式
`pi install` 后 `/pool-status` 看状态；多会话并发验证轮转

## 项目特有禁忌
- 同一会话固定同一 key，新会话才换 key
- 连续 429/quota 停止自动重试，不无限循环
